...
void SSD1306_Send_command(uint8_t data) {
    ...
    dt[0] = 0x00;
    dt[1] = data;
    CMSIS_I2C_Data_Transmit(I2C1, SSD1306_ADDR, dt, 2, 100);
}
...
void SSD1306_Send_data(uint8_t *data, uint16_t count) {
    ...
    CMSIS_I2C_Data_Transmit(I2C1, SSD1306_ADDR, dt, count + 1, 100);
}
...
void SSD1306_Generate_a_Graph(uint8_t *counter, uint8_t *array, uint8_t size_array, bool *array_is_full, int y_min, int y_max, uint8_t x_grid_time, uint8_t time_interval,
                              bool grid) {
    /// Функция, формирующая весь график, который будет отображаться на дисплее
    /// \param uint8_t *counter - счетчик значений
